...
*Amazon API Gateway | AWS Lambda | Amazon S3 | Amazon DynamoDB | Amazon Bedrock (Anthropic Claude)*

## 📋 Overview

In compliance with global regulatory standards, packaged food items are mandated to display comprehensive ingredient descriptions on their wrapper packaging. However, parsing dense jargon, synthetic preservatives, and hidden allergenic trace formulations makes it exceptionally difficult for everyday consumers to evaluate health indicators on the fly.

**NutriScan AI** resolves this problem through a completely serverless, cloud-native vision processing pipeline. A user captures a smartphone photo of a food product wrapper's ingredient label. The system handles secure object ingestion, coordinates a multimodal large language model visual evaluation via **Anthropic Claude 3.5 Sonnet**, updates a NoSQL user query index, and instantly surface-maps a concise dietary risk profiling metric graded from 1 (Low Risk) to 4 (Severe Risk).

### Key Architectural Superpowers ⚡

* **Zero Infrastructure Overhead:** The system scales dynamically during peak grocery shopping intervals and completely **scales down to zero cost** during idle traffic loops.
* **Multimodal Edge Analytics:** Avoids detached, fragile OCR software libraries by using native vision capabilities inside the model via Amazon Bedrock to extract typography and textures simultaneously.
...
## 🏗️ Production Setup & Deployment Blueprint
